...
% 
% if ~exist('chanNums','var')
%     timechan = listdlg('ListString',file_info.recChNames,...
%         'PromptString',['Which channel is Pulses? ']);
%     %Replace this with prompt file_info.recChNames
% end
%%
...
if isempty(EMGparms.NWhthreshold)
    EMGparms.NWhthreshold = EMGparms.Whthreshold;
else 
end
...
%% Plotting aligned EMGsm

% figure(1); subplot(1,2,1); hold on; axis square;
% tempep = [];
% tempt = -200/sf_down:1/sf_down:200/sf_down;
% for i = 1: length(wh_on)
%     if wh_on(i) > 201
%         figure(1); subplot(1,2,1); hold on;
%         plot(tempt,EMGsm(wh_on(i)-200:wh_on(i)+200),'k');
%         tempep = cat(1,tempep,EMGsm(wh_on(i)-200:wh_on(i)+200));
%     else
%     end
% end
% figure(1); subplot(1,2,1); hold on;
%     plot(tempt,mean(tempep),'r', 'linewidth',2);
% 
% xlabel('Duration (s)')
% ylabel('Touch epochs/average')
% 
% figure(1); subplot(1,2,2); hold on; axis square;
% tempep = [];
% tempt = -200/sf_down:1/sf_down:200/sf_down;
% for i = 1: length(nwh_on)
%     if nwh_on(i) > 201
%         figure(1); subplot(1,2,2); hold on;
%         plot(tempt,EMGsm(nwh_on(i)-200:nwh_on(i)+200),'k');
%         tempep = cat(1,tempep,EMGsm(nwh_on(i)-200:nwh_on(i)+200));
%     else
%     end
% end
% figure(1); subplot(1,2,2); hold on;
...
% 
% xlabel('Duration (s)')
% ylabel('NonTouch epochs/average')
% 
% NiceSave('AlignedTouch',figfolder,baseName)
