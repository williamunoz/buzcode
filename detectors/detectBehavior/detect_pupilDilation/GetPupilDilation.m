...
%
%New version employs different principle of pupil detection based on edge
%detection and not requiring restriction of eye area for analysis.
%DLevenstein and WMunoz 2018
...
% loading all pre-processed stuff
...
pupilmask = preprocess.pupilmask;
% shadowmask = preprocess.shadowmask;
intensitythresh = preprocess.intensitythresh;
meaneyepixel = preprocess.meaneyepixel;
stdeyepixel = preprocess.stdeyepixel;
unstablemeanthresh = preprocess.unstablemeanthresh;
unstablestdthresh = preprocess.unstablestdthresh;
...
unstablewindow = 40; %window of frames around detected unstable frames to denote as unstable
pupilsizethresh = 1; %Pupil must be larger than this many pixels
...
for ff = 1:NumberOfFrames
    ...
    meaneyepix = median(double(vidframe(eyemask))./255);
    stdeyepix = 1.4826.*mad(double(vidframe(eyemask))./255);
    ...
    if meaneyepix > unstablemeanthresh || stdeyepix < unstablestdthresh
        unstableframes = unique([unstableframes,[ff-unstablewindow:ff+unstablewindow]]);
        unstableframes(unstableframes<1 | unstableframes>NumberOfFrames)=[];
        puparea(unstableframes)=nan;
    end
    ...
    bw = edge(bwframe,'Canny',0.3); %0... for more edges...
    ...
    %Pupil hotspot
    bwframe(~pupilmask) = noneyeval;
    bwframe=~im2bw(bwframe,intensitythresh);
    tempix = find(bwframe);
    bw(tempix) = 1;
    
    if SAVEVID && mod(ff,savevidfr)==0
        subplot(4,4,10);imagesc(bw);
        set(gca,'ytick',[]);set(gca,'xtick',[]);
    end
    
    %pupil=bwmorph(pupil,'open');
    
    %Closing circle and eliminating extra stuff
    bw = imclose(bw,strel('disk',1));
    pupil = bwmorph(bw,'open');
    ...
    if N_objects < 1 || isempty(out_a) || ismember(ff,unstableframes)
        ...
        pupbox(ff,:) = out_a(pam).BoundingBox;
    end
...
puparea_raw = puparea_raw./nanmedian(puparea_raw);
